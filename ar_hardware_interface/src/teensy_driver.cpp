...
namespace ar_hardware_interface
{
                          ...
                          int num_joints, bool velocity_control_enabled)
  {
    ...
    while (!initialised_)
    {
      RCLCPP_INFO(logger_, "Waiting for response from Teensyy on port %s",
                  port.c_str());
      std::this_thread::sleep_for(std::chrono::milliseconds(1000));
      RCLCPP_INFO(logger_, "Sending init message to Teensy on port %s",
                  msg.c_str());
      exchange(msg);
    }
  ...
  bool TeensyDriver::calibrateJoints()
  {
    ...
    RCLCPP_INFO(logger_, "Calibrating joints... %s", outMsg.c_str());
